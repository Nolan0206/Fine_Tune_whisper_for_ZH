...
def update_data_csv(history_dict: dict, csv_name: list) -> dict:
    if not history_dict:
        raw = []
    else:
        raw = history_dict.get('csv', [])
    new_csv = update_list(csv_name, raw)
    return {
        "csv": new_csv
    }


def find_new_csv(history_dict: dict, csv_name: list) -> list:
    if not history_dict:
        category_list = []
    else:
        category_list = history_dict.get("csv", [])
    new_list = []
    for i in csv_name:
        if i not in category_list:
            new_list.append(i)
