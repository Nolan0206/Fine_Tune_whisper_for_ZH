...
def read_json_config(catalog_json: str):
    try:
        method, catalog_json = catalog_json.split('*')
    except ValueError:
...
def split_json(audio, ratio_train):
    seg_list, text_list, text_punc_list = [], [], []
    ...
    text_train, text_val = data_split(text_punc_list, ratio=float(ratio_train), shuffle=True)
    '''
    dataset = {
        'audio': seg_list,
        'sentence': text_list
