...
def get_whisper_hf(path_whisper_hf):
    if path_whisper_hf is None:
        ...
        try:
            # config = WhisperConfig.from_pretrained(path_whisper_hf)
            feature_extractor = WhisperFeatureExtractor.from_pretrained(path_whisper_hf)
            tokenizer = WhisperTokenizer.from_pretrained(path_whisper_hf)
            processor = WhisperProcessor.from_pretrained(path_whisper_hf)
...
def create_new_folder(path: str):
    try:
        ...
        print(Fore.BLUE + "[notice]", Fore.RED + "{}".format(path), "already exists.")
        root = "/".join(path.split('/')[0:-1])
        name = path.split("/").pop()
        folder_list = [f for f in os.listdir(root) if name in f]
        os.makedirs(f"{root}/{name}({len(folder_list)})", exist_ok=False)
